  ...
  message(content: "!list-gfcord-non-buddies") do |event|
    BuddyBot.only_creator(event.user) {
      event.bot.servers.each do |server_id, server|
        ...
        self.log "Members without Buddy for #{server.name}: #{members.map{|member| member.username + (if member.nick then ' aka ' + member.nick else '' end) + ' (' + member.id.to_s + ', joined ' + member.joined_at.tp_s + ')'}}", event.bot
      end
    }
  end
